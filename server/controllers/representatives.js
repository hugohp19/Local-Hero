...
exports.getRepByZipcode = async (req, res) => {
  try {
    ...
    // });
    const { data } = await axios({
      url: `https://cicero.azavea.com/v3.1/official?search_postal=${address}&search_country=US&format=json&key=${process.env.CICERO_API_KEY}`,
      method: 'GET'
    });
    ...
    res.status(200).json(data.response.results.candidates[0]);
  } catch (error) {
    console.log(error);
  }
};
